...
public class BitbucketConfigController {
    ...
    @Operation(
        ...
        description = "Test the connection to Bitbucket using the provided configuration"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> testConnection(
            @Parameter(description = "Bitbucket configuration to test", required = true)
            @Valid @RequestBody BitbucketConfigDTO dto) {
        
        log.info("POST /bitbucket/test-connection - Testing connection for username: {}", dto.getUsername());
        
        try {
            boolean isConnected = service.testConnection(dto);
            if (isConnected) {
                return ResponseEntity.ok(new TestConnectionResponse(true, "Connection successful", "Bitbucket API is accessible"));
            } else {
                return ResponseEntity.ok(new TestConnectionResponse(false, "Connection failed", "Unable to connect to Bitbucket API"));
            }
        } catch (Exception e) {
            log.error("Connection test failed for username: {}", dto.getUsername(), e);
