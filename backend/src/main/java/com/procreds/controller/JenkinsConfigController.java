...
public class JenkinsConfigController {
    ...
    @Operation(
        ...
        description = "Test the connection to Jenkins using the provided configuration"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> testConnection(
            @Parameter(description = "Jenkins configuration to test", required = true)
            @Valid @RequestBody JenkinsConfigDTO dto) {
        
        log.info("POST /jenkins/test-connection - Testing connection for account: {}", dto.getAccountName());
        
        try {
            boolean isConnected = service.testConnection(dto);
            if (isConnected) {
                return ResponseEntity.ok(new TestConnectionResponse(true, "Connection successful", "Jenkins API is accessible"));
            } else {
                return ResponseEntity.ok(new TestConnectionResponse(false, "Connection failed", "Unable to connect to Jenkins API"));
            }
        } catch (Exception e) {
