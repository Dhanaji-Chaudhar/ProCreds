...
public class SonarQubeConfigController {
    ...
    @Operation(
        ...
        description = "Test the connection to SonarQube using the provided configuration"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> testConnection(
            @Parameter(description = "SonarQube configuration to test", required = true)
            @Valid @RequestBody SonarQubeConfigDTO dto) {
        
        log.info("POST /sonarqube/test-connection - Testing connection for account: {}", dto.getAccountName());
        
        try {
            boolean isConnected = service.testConnection(dto);
            if (isConnected) {
                return ResponseEntity.ok(new TestConnectionResponse(true, "Connection successful", "SonarQube API is accessible"));
            } else {
                return ResponseEntity.ok(new TestConnectionResponse(false, "Connection failed", "Unable to connect to SonarQube API"));
            }
        } catch (Exception e) {
