...
public class AwsEksConfigController {
    ...
    @Operation(
        summary = "Test AWS EKS connection",
        description = "Test the connection to AWS EKS using the provided configuration"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> testConnection(
            @Parameter(description = "AWS EKS configuration to test", required = true)
            @Valid @RequestBody AwsEksConfigDTO dto) {
        
        log.info("POST /aws-eks/test-connection - Testing connection for account: {}", dto.getAccountName());
        
        try {
            boolean isConnected = service.testConnection(dto);
            if (isConnected) {
                return ResponseEntity.ok(new TestConnectionResponse(true, "Connection successful", "AWS EKS cluster is accessible"));
            } else {
                return ResponseEntity.ok(new TestConnectionResponse(false, "Connection failed", "Unable to connect to AWS EKS cluster"));
            }
        } catch (Exception e) {
