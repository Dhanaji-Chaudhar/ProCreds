...
public class AzureAksConfigController {
    ...
    @Operation(
        summary = "Test Azure AKS connection",
        description = "Test the connection to Azure AKS using the provided configuration"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> testConnection(
            @Parameter(description = "Azure AKS configuration to test", required = true)
            @Valid @RequestBody AzureAksConfigDTO dto) {
        
        log.info("POST /azure-aks/test-connection - Testing connection for account: {}", dto.getAccountName());
        
        try {
            boolean isConnected = service.testConnection(dto);
            if (isConnected) {
                return ResponseEntity.ok(new TestConnectionResponse(true, "Connection successful", "Azure AKS cluster is accessible"));
            } else {
                return ResponseEntity.ok(new TestConnectionResponse(false, "Connection failed", "Unable to connect to Azure AKS cluster"));
            }
        } catch (Exception e) {
